...
### フェーズ2: クロロフィルも GCOM-C に一本化（実装済み）

`sst.source: gee` のとき、クロロフィルも **GCOM-C（`CHLA_AVE`×0.0016）を SST と同じ
観測日**で取得します。**同一センサ・同一日・同一格子**なので「潮目（水温フロント）×
餌場（クロロフィル濃淡）」の重ね合わせが物理的に正しく成立します。

- 海色は太陽光が要るため**昼間パス（`SATELLITE_DIRECTION=D`）**のみ
- GCOM-C は雲で欠測する（補間しない）→ **雲域は無色**（旧 CMEMS の「補間値」とは別）
- クロロフィル勾配は**雲の大穴を埋めない**ガード付き（有効近傍が乏しい所は計算しない）
- QA フラグ（陸率ビット 0=海/1=ほぼ海/2=沿岸/3=陸）は分布をログ出力。沿岸の
  半透明化は当面「海岸線からの距離」側に任せる（QA 併用の是非は実データで判断）
